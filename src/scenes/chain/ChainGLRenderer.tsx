...
interface ChainGLRendererProps {
  sim: ChainSimulation;
  onStats?: () => void;
}

export function ChainGLRenderer({ sim, onStats }: ChainGLRendererProps) {
  const { gl: renderer, size } = useThree();
  const stateRef = useRef<ReturnType<typeof createChainGL> | null>(null);

  useEffect(() => {
    const gl = renderer.getContext() as WebGLRenderingContext;
    stateRef.current = createChainGL(gl);
    return () => {
  ...
  useEffect(() => {
    ...
    stateRef.current?.resize(renderer.domElement.width, renderer.domElement.height);
  }, [sim, size.width, size.height, renderer]);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      last = now;
      sim.step(dt);
      stateRef.current?.render(sim, now / 1000);
      onStats?.();
    };
...
function createChainGL(gl: WebGLRenderingContext) {
  ...
  function drawPoints(data: Float32Array, color: [number, number, number], cssW: number, cssH: number) {
    const n = data.length / 4;
    if (!n) return;
    gl.useProgram(pointProg);
    gl.uniform2f(U.ptRes, cssW, cssH);
  ...
  return {
    ...
    render(sim: ChainSimulation, time: number) {
      ...
      if (sim.neutrons.length) {
        ...
        drawPoints(d, [0.62, 0.78, 1.0], cssW, cssH);
      }
      ...
      if (sim.neutrons.length) {
        ...
        drawPoints(d, [1.0, 1.0, 1.0], cssW, cssH);
      }
      gl.disable(gl.BLEND);
    },
  };
